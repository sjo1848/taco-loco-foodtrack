...
export async function GET() {
  try {
    await requireAdmin();
    return NextResponse.json(await catalogService.listCategories());
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    await requireAdmin();
    return NextResponse.json(await catalogService.createCategory(await request.json()), { status: 201 });
  } catch (error) {
    if (error instanceof Error && error.message === "UNAUTHORIZED") return NextResponse.json({ code: "UNAUTHORIZED" }, { status: 401 });
