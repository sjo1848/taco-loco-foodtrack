...
import { buildWhatsAppMessage, addSelectionLine, selectionCount, selectionTotal, updateSelectionQuantity, type SelectionLine } from "@/modules/selection/model";
...
type Menu = { settings: { businessName: string; whatsappUrl: string }; categories: Array<{ id: string; name: string; slug: string; products: Product[] }> };
...
export function MenuExperience({ menu }: { menu: Menu }) {
  ...
  function confirmProduct() {
    ...
    setLines((current) => addSelectionLine(current, { productId: pendingProduct.id, name: pendingProduct.name, priceAmount: pendingProduct.priceAmount, modifiers }));
  ...
  return <main className="public-menu"><BrandHeader businessName={menu.settings.businessName} /><CategoryNav categories={menu.categories} />
    ...
    <div className="whatsapp-cta"><a className="whatsapp-cta__button" href={whatsappHref} rel="noreferrer" target="_blank"><span aria-hidden="true">◉</span><span>{count > 0 ? "Enviar pedido por WhatsApp" : "Pedir por WhatsApp"}</span></a></div>
    {pendingProduct && <div className="selection-overlay" role="presentation"><section className="selection-sheet" role="dialog" aria-modal="true" aria-labelledby="selection-title"><div className="selection-sheet__header"><div><p className="eyebrow">Personalizá tu pedido</p><h2 id="selection-title">{pendingProduct.name}</h2></div><button className="selection-close" type="button" onClick={() => setPendingProduct(null)} aria-label="Cerrar">×</button></div>{pendingProduct.modifiers.map((group) => <fieldset className="modifier-group" key={group.name}><legend>{group.name}{group.required ? " · obligatorio" : ""}</legend><div className="modifier-options">{group.options.map((option) => { const checked = (selectedOptions[group.name] ?? []).includes(option); return <button className={`modifier-option${checked ? " modifier-option--selected" : ""}`} type="button" aria-pressed={checked} key={option} onClick={() => toggleOption(group, option)}>{option}</button>; })}</div></fieldset>)}<button className="button selection-confirm" type="button" onClick={confirmProduct}>Agregar al pedido · {formatPrice(pendingProduct.priceAmount)}</button></section></div>}
    {showSummary && <div className="selection-overlay" role="presentation"><section className="selection-sheet" role="dialog" aria-modal="true" aria-labelledby="summary-title"><div className="selection-sheet__header"><div><p className="eyebrow">Tu selección</p><h2 id="summary-title">Revisá tu pedido</h2></div><button className="selection-close" type="button" onClick={() => setShowSummary(false)} aria-label="Cerrar">×</button></div>{lines.length === 0 ? <p className="selection-empty">Todavía no agregaste productos.</p> : <div className="selection-lines">{lines.map((line) => <div className="selection-line" key={line.id}><div><strong>{line.name}</strong>{line.modifiers.length > 0 && <small>{line.modifiers.map((modifier) => `${modifier.group}: ${modifier.option}`).join(" · ")}</small>}<span>{formatPrice(line.priceAmount)} c/u</span></div><div className="quantity-control" aria-label={`Cantidad de ${line.name}`}><button type="button" onClick={() => setLines((current) => updateSelectionQuantity(current, line.id, line.quantity - 1))} aria-label={`Quitar una unidad de ${line.name}`}>−</button><strong>{line.quantity}</strong><button type="button" onClick={() => setLines((current) => updateSelectionQuantity(current, line.id, line.quantity + 1))} aria-label={`Agregar una unidad de ${line.name}`}>+</button></div></div>)}</div>}<div className="selection-summary"><span>Total informativo</span><strong>{formatPrice(total)}</strong></div><div className="selection-actions"><button className="button button--secondary" type="button" onClick={() => setShowSummary(false)}>Seguir viendo el menú</button><a className="button" href={whatsappHref} rel="noreferrer" target="_blank">Enviar por WhatsApp</a></div></section></div>}
  </main>;
}
