...
export function ProductCard({ product, featured = false, onAdd }: { product: Product; featured?: boolean; onAdd?: (product: Product) => void }) {
  const imageUrl = product.imageKey ? `/api/media/products/${product.imageKey.split("/").map(encodeURIComponent).join("/")}` : null;
  return <article className={`product-card${featured ? " product-card--featured" : ""}${!product.available ? " product-card--sold-out" : ""}`}>
    <div className="product-card__content">
      ...
      <div className="product-card__footer"><p className="product-card__price">{formatPrice(product.priceAmount)}</p>{onAdd && <button className="product-card__add" type="button" disabled={!product.available} onClick={() => onAdd(product)}>{product.available ? "Agregar" : "Agotado"}</button>}</div>
    </div>
